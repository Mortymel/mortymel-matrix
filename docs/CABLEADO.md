...
Esta guía corresponde a **una matriz HUB75 64 × 64, barrido 1/32, con dirección E**. La tabla principal da el mapa inicial de **ESP32-S3**; al final figura el mapa de ESP32 clásico y S2. Mira la serigrafía y la hoja técnica de *tu panel*: existen variantes de barrido, controladores y conectores. Desconecta la fuente y el USB antes de mover cualquier cable.

## Material y alimentación

- ESP32 clásico, S2 o S3 con GPIO accesibles y un firmware compatible detectado por el [instalador](INSTALACION.md); matriz HUB75 con conector de entrada **IN**, cable HUB75/IDC correcto y una fuente **5 V externa** dimensionada para la corriente indicada por el fabricante del panel.
- Conecta la fuente 5 V al **conector de alimentación del panel**, no a un pin de señales ni a un GPIO. Conecta el GND de esa fuente al GND del panel **y** al GND del ESP32: todas las señales necesitan una referencia común. Alimenta el ESP32 mediante su USB o un circuito adecuado para su placa. No alimentes los LEDs desde el pin 5 V del microcontrolador.
...
## Otros chips: mapas iniciales de la biblioteca

El instalador reconoce ESP32 clásico y ESP32-S2. La distribución de pines de la **placa física** sigue siendo diferente en cada modelo: consulta siempre su esquema. Las siguientes son asignaciones iniciales de la biblioteca, **no instrucciones para conectarlas sin comprobarlas**. E empieza en `-1` en todos los perfiles.

| Señal | ESP32 clásico | ESP32-S2 |
| --- | ---: | ---: |
| R1 | 25 | 45 |
| G1 | 26 | 42 |
| B1 | 27 | 41 |
...
En ciertas placas S2, GPIO 26 se reserva para la flash: **remapea LAT desde la web antes de conectar el panel**. El firmware impide guardar combinaciones con GPIO reservados según la familia, aunque no conoce qué pines usa tu placa concreta. En ESP32 clásico también hay pines de arranque en el mapa inicial; verifica sus niveles al arrancar. Consulta los [valores de la biblioteca para ESP32](https://github.com/mrcodetastic/ESP32-HUB75-MatrixPanel-DMA/blob/cf09801a362c71fc61da99751972b6dfb6e63b63/src/platforms/esp32/esp32-default-pins.hpp) y [S2](https://github.com/mrcodetastic/ESP32-HUB75-MatrixPanel-DMA/blob/cf09801a362c71fc61da99751972b6dfb6e63b63/src/platforms/esp32s2/esp32s2-default-pins.hpp).

## Adaptar el mapa a tu placa

1. Instala el firmware con **la matriz desconectada**. Lee los GPIO disponibles en el esquema o pinout de la placa exacta. La capacidad de flash 4/8/16 MB, detectada automáticamente, no determina qué pines están expuestos.
2. En la consola web local abre **Sistema → Conexión HUB75**. Ajusta las 14 señales si tu cableado requiere otros GPIO. En S3 se rechazan pines repetidos, GPIO 0, 19–20, 22–37, 43–46 y los GPIO inexistentes; los otros chips usan una validación propia. Algunas placas reservan más pines: compruébalo tú. GPIO 3 figura por compatibilidad con el mapa inicial de la biblioteca; si afecta al arranque, cambia C a otro GPIO libre *antes de conectar el panel*.
...
**Evita pines comprometidos con flash, PSRAM, USB, UART y arranque.** GPIO 19/20 son USB nativo en S3/S2; algunos GPIO dependen de la variante de memoria. En S3 este firmware reserva conservadoramente 22–37 para no interferir con memorias de diversas placas. La interfaz no puede detectar un cable invertido, un panel de otro barrido o una fuente insuficiente.
